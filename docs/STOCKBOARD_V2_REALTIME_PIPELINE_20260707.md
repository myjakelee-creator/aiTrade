...
이 문서는 StockBoard v2의 현재 실시간 구조, 선발모델, 5분강도, 잔량비, 행 위치 정책과 운영 검증 상태를 기록하는 단일 기준 문서이다. 기존 StockBoard v0.3.x 문서와 섞지 않는다.

## 1. 현재 결론

StockBoard v2는 32비트 Kiwoom OpenAPI collector와 64비트 worker를 분리해 장개시 이벤트 폭주 구간의 수신 속도와 표시 안정성을 확보한다.

현재 완료·연결 상태:

- 현재가·등락률·거래대금 실시간 표시
- 5분강도 opt10046 차등 조회·당일 복원·정규장 마감 보충
- 잔량비 opt10004 저속 조회·마지막 정상값 유지
- Top20·Top300 내부 행 위치 고정
- 안전장치를 통과한 Top20 ↔ Top300 승강
- 하드코딩 기준을 제외한 8개 설정형 선발모델 v3 실행
- 미지원 항목을 조용히 0점 처리하지 않는 설정 검증기
- 실제 Top300 → Top50 → Top20 → Top5 Funnel
- HTML 계산 금지, worker 계산 결과 표시 전용
...
브라우저는 `http://127.0.0.1:8765/`에서 `Ctrl+F5`로 갱신한다.
...
| 대형 수집 | `realtime_v2/collector32_large.py` | 대량체결 집계·5분강도 정상값 저장 |
| 잔량비 wrapper | `realtime_v2/collector32_large_bidask.py` | 잔량비 scheduler 설치 |
| worker | `realtime_v2/worker64_guarded_large.py` | 상태·5분강도·대량체결·UI 결과 제공 |
...
| 설정 검증 | `stockboard_candidate_config.py` | registry·설정키·가중치 검증 |
| 공통 파생값 | `stockboard_candidate_features.py` | 승인 원천에서 공통 지표 1회 계산 |
| 모델 실행 | `stockboard_candidate_engine.py` | 점수·등급·Coverage·실제 Funnel 계산 |
...
- 데이터 수집·점수 계산을 브라우저로 이동하지 않는다.
- 선택된 모델 하나만 worker에서 계산한다.
- 신규 OpenAPI 조회를 선발모델 때문에 추가하지 않는다.
- `data/runtime/`은 Git 추적 금지이다.

## 4. 시장시간 정책

시간은 `config/stockboard_market_calendar.json` 기준이며 `special_days`가 우선한다.
...
| after_wait | 15:30~15:40 | 정규장 마감 보충 |
| aftermarket | 15:40~20:00 | NXT 새 값 우선, 미거래 종목 마감값 유지 |
...
| 새 거래일 08:00 전 재시작 | 전일값 미복원, `-` |
| 프로세스가 날짜를 넘어 계속 실행 | 메모리값 유지 후 프리마켓 새 값으로 교체 |
| 0·빈값·오류 | 정상값을 덮어쓰지 않음 |
| 15:30 이후 NXT 미거래 | 정규장 마감 5분강도 보충 |

조회 우선순위는 `S1 → Top20 → Hidden Top50 → Top300`이다. 다른 TR이 진행 중이면 양보한다. 마감 보충 실패는 종목별 최대 2회만 재시도한다.

## 6. 잔량비 정책

잔량비는 화면에 유지하지만 **현재 모든 선발모델 점수에서는 제외**한다.

- opt10004 단건 저속 조회
- 마지막 정상 양수값 cache
- 0·빈값·오류가 정상값을 덮어쓰지 않음
- 09:00~09:10 Hidden50·Top300 신규조회 제한
- 향후 신뢰성과 속도 검증 후 새 모델 버전에서만 재검토

## 7. 선발모델 v3 공통 원천

점수에 사용할 수 있는 화면·worker 원천은 다음으로 제한한다.
...
설정에 미지원 키가 있으면 해당 모델을 `INVALID`로 판정하고 드롭다운에서 제외한다. 미지원 키를 0점으로 계산하지 않는다.

## 8. 등록 모델

하드코딩 `TVRANK_A_V03_TEMP`는 registry와 파일에서 제거했다.

| 모델 | 핵심 목적 | 상태 |
|---|---|---|
| 5요소 수급선발 v0.1 | 순위상승·대금비·강도·프로·대량체결 조합 | READY |
| 순매수 강도 v0.2 · 5분강도 | 프로그램·대량체결 비중 강화 | READY |
| 순매수 강도 v0.1 · 5분강도 | 7개 수급 요소 균형 | READY |
| 돈쏠림 시작형 · 5분강도 | 순위·대금비가 막 상승하는 종목 | READY |
...
각 모델은 `final_score`, `entry_score`, `confirmation_score`, `focus_score`를 가진다. 각 단계의 양수 가중치 합은 100이어야 하며 음수 가중치는 승인된 penalty 항목만 허용한다.
...
모델별 guard는 목적에 반하는 상황에서 최고점을 제한한다. 예: 프로그램 동행형의 프로그램 순매도는 59점 이하이다.
...
- 선택된 모델 한 개만 점수 계산한다.
- 최대 300행의 정렬 3회 수준이며 OpenAPI 수신경로와 분리한다.
- HTML은 계산하지 않는다.
- 신규 sidecar·launcher·TR scheduler를 만들지 않는다.
- 5분강도와 잔량비의 기존 저속 조회정책은 유지한다.
...
```text
/api/v2/candidate_models 에 8개 모델만 표시
선발기준 변경 시 candidate_model_id 변경
entry_rank / confirmation_rank / focus_rank 존재
display_order.mode = lane_stable
점수 변화에도 lane 내부 행 위치 유지
```

## 13. 남은 실전 검증

- 다음 프리마켓·정규장에서 8개 모델을 순차 선택해 점수 분포 확인
- A/B 등급 종목의 HTS 체결·프로그램·대량체결 상태 대조
- 모델 전환 시 worker 계산시간과 stream latency 확인
- 필요 시 배점은 JSON 설정만 조정하고 수집경로는 변경하지 않음
