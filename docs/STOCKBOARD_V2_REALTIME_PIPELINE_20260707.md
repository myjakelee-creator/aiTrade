...
### 7.9 UI 버전 식별과 안정판 봉인

화면 상단에는 실행 버전과 작업 키워드를 항상 표시한다. 비공개 8765와 공개 8767은 같은 식별자를 사용한다.

현재 검증 완료 식별자:

```text
UI_VERSION       SBV2-20260722.2
KEYWORD          PUBLIC-OPS-ID
VERIFIED_COMMIT  6e48d6dce34f995770a26aacec30b7e5621e0889
STABLE_BRANCH    stable/SBV2-20260722.2
```

버전명 표시 규칙:

- 버전명 자체에 `YYYYMMDD` 날짜가 포함되면 화면에서 버전명 뒤에 날짜·시간을 다시 붙이지 않는다.
- 버전명에 날짜가 없을 때만 날짜를 별도로 표시한다.
- 시간은 버전명 중복 방지를 위해 기본 화면 표시에서 제외하고, 필요하면 tooltip·문서에서만 확인한다.
- 키워드는 해당 버전의 핵심 목적을 짧게 표시한다.
- 안정 브랜치는 검증 기준점이며 이후 개발 커밋으로 이동시키지 않는다.
- 복원 기준은 브랜치명보다 불변 전체 커밋 SHA를 우선한다.

2026-07-22 12:01 KST 대표님 화면에서 가격·거래대금·보조지표·비공개 UI와 버전 표시를 확인했다. 이 시점의 코드를 `stable/SBV2-20260722.2`로 봉인했으며, 이후 수정은 별도 hotfix·feature 브랜치에서만 진행한다.
...
### 10.6 2026-07-22 12:01 KST 안정판 확인

- 비공개 StockBoard 상단에서 `SBV2-20260722.2 · PUBLIC-OPS-ID` 식별자를 확인했다.
- 현재가·등락률·거래대금·대금비·1분대금·일봉·잔량비·체결강도·5분강도·프로그램·대량체결이 함께 표시되는 것을 확인했다.
- 이 화면의 정확한 코드 상태는 `6e48d6dce34f995770a26aacec30b7e5621e0889`이다.
- 안정 브랜치 `stable/SBV2-20260722.2`를 이 커밋에 고정했다.
- 장중 실행 프로세스·가격 수집·Worker·공개 서비스는 재시작하거나 변경하지 않았다.
...
이 복원 명령은 코드 기준점만 복원한다. runtime 상태는 거래일·state schema가 호환되는지 별도 확인한 뒤 사용한다.

2026-07-22 12:01 KST 기준으로 `SBV2-20260722.2` UI 식별, 가격·거래대금·보조지표 표시와 비공개 운영 화면을 확인하고 정확한 커밋 `6e48d6d`를 안정 브랜치로 봉인했다. 이후 작업은 안정 브랜치를 직접 수정하지 않고 별도 hotfix·feature 브랜치에서만 진행한다.
