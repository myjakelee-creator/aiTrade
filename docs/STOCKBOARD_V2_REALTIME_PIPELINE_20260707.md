...
## 15. 2026-07-16 통합 6지표 수명주기 정책

이 절은 대금비·잔량비·체결강도·5분강도·프로(억)·대량체결의 현재 운영정책이며, 위의 개별 cache 설명과 충돌할 경우 이 절이 우선한다.

| 지표 | 장중 원천 | 장마감·다음 프리마켓 전 | 0·결측 정책 |
|---|---|---|---|
| 대금비 | worker의 `trade_value_eok / prev_trade_value_eok` | 같은 거래일 정규장 최종값 | 분모 없음·날짜 불명은 `-` |
| 잔량비 | Kiwoom REST `ka10004` | 직전 거래일 최종 정상값 | 0·날짜 불명·과거 오염값은 `-` |
| 체결강도 | Kiwoom WebSocket 주식체결 `0B`, FID `228` | 마지막 정상 FID228 | `ka10046` 값을 체결강도로 사용 금지 |
| 5분강도 | Kiwoom REST `ka10046` | 직전 거래일 최종 정상값 | 0·날짜 불명은 `-` |
| 프로(억) | Kiwoom REST `ka90004` | 직전 거래일 최종값 | 원천·날짜가 있으면 실제 0 허용 |
| 대량체결 | 저속 `ka10055`, 5천만원 기준 signed 체결 | 직전 거래일 최종 누적값 | 원천·날짜가 있으면 실제 0 허용 |
...
날짜 불명           표시 금지
결측 null/빈문자    0으로 변환하지 않고 '-'
최종 표시 권한      worker_six_metric_lifecycle_patch + output_guard
저장 파일           data/runtime/stockboard_v2/six_metric_lifecycle.json
저장 간격           값 변경 시 최대 5초당 1회
```

성능 보호:

- 생산 가격 collector는 `FID 10;12;20;14`를 유지한다.
- FID15와 두 번째 QAx를 추가하지 않는다.
- REST는 단일 background thread와 최소 2초 간격을 공유한다.
- 체결강도 WebSocket은 S1 한 종목만 구독하고 최대 1Hz로 반영한다.
- 09:00~09:10 대량체결은 S1만 180초 주기로 조회한다.
- 브라우저는 계산하지 않고 결측 포맷과 `체결강도` 명칭만 표시한다.
