...
- 모바일 행 클릭은 종목 선택과 강조만 수행하고 PC HTS 클립보드 명령은 전송하지 않는다.
- 상단에는 화면 전환, 시계, 연결·장 상태, 선발기준, StockBoard·ThemeBoard 링크와 모멘텀 알림만 유지한다.
- StrategyBoard span, 새 창 버튼, 화면 확대·폭 최소화·행 위치·HTS 안내와 모든 속도·queue·drop·render 정보는 모바일에서 숨긴다.
- ThemeBoard 링크·경로·데이터 허브·선택 상세·CSS 파일은 수정하지 않으며 모바일 hide selector에도 ThemeBoard를 포함하지 않는다.
- 미국시장과 국내시장 숫자표는 컨테이너 폭을 넘으면 오른쪽 열부터 완전히 숨기고, 최소 핵심 열은 유지한다.
- 국내시장 수급·등락 그래프 4개는 모바일에서 2×2로 배치해 오른쪽 잘림을 방지한다.
- 기존 fast-price DOM 패치는 모바일에서 현재가 셀을 건너뛰고 모바일 `등락률` 셀 인덱스만 갱신하도록 분기한다.
- 신규 QAx·FID·Kiwoom REST·WebSocket·Worker thread·SSE payload는 0이며, 모바일에서는 행당 DOM 셀이 감소한다.
...
- 모바일 전용 8열 DOM 렌더러와 비율 열폭 구현
- 모바일 fast-price 인덱스 보호, HTS 클립보드 명령 차단 구현
- 속도·queue·drop·render 정보와 StrategyBoard·새 창 숨김 구현
- ThemeBoard 링크 유지와 ThemeBoard 선택자·데이터 경로 비접촉 회귀 고정
- 미국·국내시장 오른쪽 초과 열 자동 숨김과 국내 그래프 2×2 구현
- StockBoard CI Run #301 Windows regression 성공
- Targeted pytest 106 passed / 0 failed
...
5. 브라우저 폭 760px 왕복에서 모바일↔데스크톱 자동 전환과 버튼 강제 전환 확인
6. 모바일 8열, StrategyBoard 숨김, ThemeBoard 유지, 미국·국내시장 오른쪽 잘림 없음 확인
7. 휴대전화 세로·가로 회전과 360~760px 폭에서 표·상단 모멘텀 알림 확인
8. 09:00~09:10 개장 폭주에서 queue·drop·stream·stale·render 확인
9. 잔량비 3회전 이상 후 Top100 최종 커버리지 측정
10. 15:30·20:00·자정·익일 프리마켓 rollover 확인
11. NXT 거래·미거래 종목의 마지막 정상값 유지 확인
