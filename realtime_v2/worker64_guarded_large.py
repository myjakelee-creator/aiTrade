...
def _apply_large_trade_delta(state, code: str, quote: dict[str, Any], delta: dict[str, Any], received_at: Any) -> None:
    ...
    quote["large_trade_net_sum_eok"] = round(float(quote.get("large_trade_buy_sum_eok") or 0.0) - float(quote.get("large_trade_sell_sum_eok") or 0.0), 4)
...
def _speed_render_patch(html: str) -> str:
    ...
    patch = r"""
  /* STOCKBOARD_V2_LARGE_SPEED_PATCH: display-only rendering, navigation, and metric toggle fixes. */
  deriveClientFields = function(row){ return row && typeof row === 'object' ? {...row} : {}; };

  const __largeSpeedRenderCache = new WeakMap();
  let __largeSpeedForceFullPoolRepaint = false;
  let __largeSpeedLastNavigationTable = null;

  const __largeSpeedBaseToggleMetricMode = typeof toggleMetricMode === 'function' ? toggleMetricMode : null;
  if(__largeSpeedBaseToggleMetricMode){
    toggleMetricMode = function(key){
      __largeSpeedForceFullPoolRepaint = true;
      const result = __largeSpeedBaseToggleMetricMode(key);
      requestAnimationFrame(() => { __largeSpeedForceFullPoolRepaint = false; });
      return result;
    };
  }

  function __largeSpeedMetricModeSignature(){
    try { return Array.from(metricKeys || []).map(key => `${key}:${metricModes[key] || 'number'}`).join(','); }
    catch(_e) { return ''; }
  }
  ...
  function __largeSpeedRowSignature(row){
    ...
    const keys = ['stock_code','rank','rank_change','candidate_grade_text','grade_text','grade','candidate_grade','grade_score','candidate_score','score_percent','stock_name','price','change_rate','trade_value_eok','prev_trade_value_eok','amount_ratio','ohlc','realtime_ohlc','display_ohlc','bid_ask_ratio','execution_strength','strength_1m','program_net','large_trade_net_count','large_trade_buy_count','large_trade_sell_count','large_trade_buy_sum_eok','large_trade_sell_sum_eok','large_trade_net_sum_eok','price_age_sec'];
  ...
  function __largeSpeedMakeRow(row, options){
    ...
    if(!next) return null;
    if(options && options.suppressFlash) next.querySelectorAll('.cell-flash').forEach(cell => cell.classList.remove('cell-flash'));
    if(options && options.suppressTitle){ next.removeAttribute('title'); next.querySelectorAll('[title]').forEach(cell => cell.removeAttribute('title')); }
    return next;
  }
  ...
  function __largeSpeedPatchRow(existing, next){
    ...
    if(oldCells.length !== newCells.length){ existing.replaceWith(next); return next; }
    for(let i = 0; i < newCells.length; i += 1){
      if(oldCells[i].outerHTML !== newCells[i].outerHTML) oldCells[i].replaceWith(newCells[i].cloneNode(true));
    }
    return existing;
  }
  function __largeSpeedUpdateWindow(order, cache, budget, priorityCodes){
    const size = order.length;
    if(!Number.isFinite(budget) || budget <= 0 || budget >= size) return new Set(order);
    const result = new Set(priorityCodes || []);
    const start = Math.max(0, Math.min(size - 1, Number(cache.cursor || 0)));
    for(let offset = 0; offset < budget && offset < size; offset += 1) result.add(order[(start + offset) % size]);
  ...
  function __largeSpeedRenderTable(table, rows, empty, options = {}){
    ...
    if(!cache){ cache = {nodes:new Map(), signatures:new Map(), order:[], empty:false, sortKey:'', cursor:0}; __largeSpeedRenderCache.set(table, cache); }
    if(!nextRows.length){
      if(!cache.empty){ tb.innerHTML = `<tr><td colspan="${columns.length}" class="center">${escapeHtml(empty)}</td></tr>`; cache.nodes.clear(); cache.signatures.clear(); cache.order = []; cache.cursor = 0; cache.empty = true; }
      return;
    }
    cache.empty = false;
    const rowsByCode = new Map();
    const incomingOrder = [];
    nextRows.forEach((row, index) => { const code = String(row && row.stock_code || `__row_${index}`); if(!rowsByCode.has(code)) incomingOrder.push(code); rowsByCode.set(code, row); });
    const sortKey = `${sortState.key || ''}/${sortState.dir || ''}`;
    if(options.stableOrder && cache.sortKey !== sortKey){ cache.order = []; cache.cursor = 0; cache.sortKey = sortKey; }
    if(!options.stableOrder){ cache.order = incomingOrder.slice(); cache.cursor = 0; cache.sortKey = sortKey; }
    else { const stillVisible = new Set(incomingOrder); cache.order = cache.order.filter(code => stillVisible.has(code)); incomingOrder.forEach(code => { if(!cache.order.includes(code)) cache.order.push(code); }); }
    const priorityCodes = [];
    if(selectedCode) priorityCodes.push(String(selectedCode));
    const updateBudget = __largeSpeedForceFullPoolRepaint ? 0 : Number(options.maxUpdateRows || 0);
    ...
    cache.order.forEach(code => {
      ...
      if(!node || cache.signatures.get(code) !== signature){
        if(!node || updateCodes.has(code)){ const nextNode = __largeSpeedMakeRow(row, options); if(!nextNode) return; node = node ? __largeSpeedPatchRow(node, nextNode) : nextNode; cache.nodes.set(code, node); cache.signatures.set(code, signature); }
      }
      if(node) desiredNodes.push(node);
    });
    for(const code of Array.from(cache.nodes.keys())){ if(!used.has(code)){ cache.nodes.delete(code); cache.signatures.delete(code); } }
    const currentNodes = Array.from(tb.children);
    const sameDomOrder = currentNodes.length === desiredNodes.length && desiredNodes.every((node, index) => currentNodes[index] === node);
    if(!sameDomOrder) tb.replaceChildren(...desiredNodes);
  }
  renderTable = function(table, rows, empty){
    if(table === poolBoardEl) return __largeSpeedRenderTable(table, rows, empty, {stableOrder:true, suppressFlash:true, suppressTitle:true, maxUpdateRows:50});
    return __largeSpeedRenderTable(table, rows, empty, {stableOrder:false, suppressFlash:false, suppressTitle:false, maxUpdateRows:0});
  };
  ...
  function __largeSpeedIsBoardTable(table){ return table === selectedBoardEl || table === focusBoardEl || table === poolBoardEl; }
  function __largeSpeedTableHasCode(table, code){ return !!(table && code && table.querySelector && table.querySelector(`tbody tr[data-code="${code}"]`)); }
  function __largeSpeedRememberNavigationTable(event){ const row = event && event.target && event.target.closest ? event.target.closest('tr[data-code]') : null; const table = row ? row.closest('table') : null; if(__largeSpeedIsBoardTable(table)) __largeSpeedLastNavigationTable = table; }
  ...
  function __largeSpeedNavigationTable(){
    ...
    if(selectedCode){ const preferred = [__largeSpeedLastNavigationTable, focusBoardEl, poolBoardEl, selectedBoardEl]; for(const table of preferred){ if(__largeSpeedIsBoardTable(table) && __largeSpeedTableHasCode(table, selectedCode)) return table; } }
  ...
  function __largeSpeedRowsInTable(table){ const body = table && table.tBodies ? table.tBodies[0] : null; return Array.from(body ? body.querySelectorAll('tr[data-code]') : []).filter(row => /^\d{6}$/.test(String(row.dataset.code || ''))); }
  function __largeSpeedFocusCodeInTable(table, code){ const row = table && table.querySelector ? table.querySelector(`tbody tr[data-code="${code}"]`) : null; if(row && typeof row.focus === 'function'){ row.focus({preventScroll:true}); if(typeof row.scrollIntoView === 'function') row.scrollIntoView({block:'nearest', inline:'nearest'}); return true; } return false; }
  ...
  function __largeSpeedBoardWidth(){ const cssWidth = Number.parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--board-width')) || 0; const tableWidths = Array.from(document.querySelectorAll('table.board')).map(table => Math.max(table.scrollWidth || 0, table.getBoundingClientRect().width || 0)); return Math.max(cssWidth, ...tableWidths, 0); }
  function __largeSpeedApplyHorizontalScrollFix(){ const boardWidth = __largeSpeedBoardWidth(); const minWidth = Math.ceil(Math.max(window.innerWidth + 180, boardWidth + 240)); const windowEl = document.querySelector('.window'); if(windowEl) windowEl.style.minWidth = `${minWidth}px`; document.documentElement.style.minWidth = `${minWidth}px`; document.body.style.minWidth = `${minWidth}px`; }
  const __largeSpeedBaseUpdateBoardWidth = typeof updateBoardWidth === 'function' ? updateBoardWidth : null;
  if(__largeSpeedBaseUpdateBoardWidth){ updateBoardWidth = function(...args){ const result = __largeSpeedBaseUpdateBoardWidth.apply(this, args); requestAnimationFrame(__largeSpeedApplyHorizontalScrollFix); return result; }; }
  ...
  render = function(...args){ const result = __largeSpeedBaseRender.apply(this, args); __largeSpeedUpdateRenderDiagnostics(); requestAnimationFrame(__largeSpeedApplyHorizontalScrollFix); return result; };
"""
