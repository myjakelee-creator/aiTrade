...
def test_same_day_cumulative_value_regression_keeps_latest_price_and_previous_amount():
    _run(
        r'''
...
class State:
    def __init__(self):
        ...
        self.quotes={"000660":{"stock_code":"000660","row_source":"realtime","price":100,"trade_value_eok":100.0,"trade_value_trading_date":"20260722","_trade_time_seconds":101500,"trade_time":"101500"}}
...
install(Base)
state=State()
state._apply_trade({"type":"trade","ts":"2026-07-22T10:15:01+09:00","stock_code":"000660","received_code":"000660_AL","kwargs":{"price":"110","trade_time":"101501","cumulative_value":"90","source_code":"000660_AL","raw":{"price_raw":"110","trade_time_raw":"101501","cumulative_value_raw":"90"}}})
assert state.quotes["000660"]["price"]==110.0
assert state.quotes["000660"]["trade_value_eok"]==100.0
assert state.quotes["000660"]["trade_value_trading_date"]=="20260722"
assert state.status["trade_field_regression_suppressed_reason_counts"]=={"cumulative_trade_value_decreased":1}
assert state.status.get("daily_cumulative_reset_accepted_count",0)==0
...
class State:
    def __init__(self):
        self.lock=threading.RLock()
        self.status={"trade_count":0,"market_trading_date":"20260722"}
        self.quotes={"000660":{"stock_code":"000660","row_source":"realtime","price":100,"change_rate":1.0,"trade_value_eok":100.0,"trade_value_trading_date":"20260722","_trade_time_seconds":101500,"trade_time":"101500"}}
    def _apply_trade(self,event):
        values=merged(event); quote=self.quotes[event["stock_code"]]
...
install(Base)
state=State()
state._apply_trade({"type":"trade","ts":"2026-07-22T10:15:02+09:00","stock_code":"000660","received_code":"000660_AL","kwargs":{"price":"111","change_rate":"2.0","trade_time":"101459","cumulative_value":"99","source_code":"000660_AL"}})
...
class State:
    def __init__(self):
        self.lock=threading.RLock()
        self.status={"trade_count":0,"market_trading_date":"20260722"}
        self.quotes={"000660":{"stock_code":"000660","row_source":"realtime","price":100,"trade_value_eok":144298.0,"_trade_time_seconds":80001,"trade_time":"080001"}}
    def _apply_trade(self,event):
        values=merged(event); quote=self.quotes[event["stock_code"]]
        quote["price"]=float(values.get("price"))
        cumulative=values.get("trade_value_eok")
        if cumulative not in (None,""): quote["trade_value_eok"]=float(cumulative)
        self.status["trade_count"]+=1
class Base:
    State=State; merged_event_values=staticmethod(merged)
install(Base)
state=State()
state._apply_trade({"type":"trade","ts":"2026-07-22T08:00:02+09:00","stock_code":"000660","received_code":"000660_AL","kwargs":{"price":"101","trade_time":"080002","trade_value_eok":"6413","source_code":"000660_AL"}})
assert state.quotes["000660"]["price"]==101.0
assert state.quotes["000660"]["trade_value_eok"]==144298.0
assert state.status["trade_field_regression_suppressed_reason_counts"]=={"cumulative_trade_value_decreased":1}
assert state.status.get("daily_cumulative_reset_accepted_count",0)==0
'''
    )
...
class State:
    def __init__(self):
        self.lock=threading.RLock(); self.status={"trade_count":0,"market_trading_date":"20260722"}
    ...
    def _apply_trade(self,event):
        ...
        if incoming is not None and incoming < quote["_trade_time_seconds"]:
            fake_guarded._drop_trade(self,quote,event["stock_code"],"older_fid20_than_last_accepted",event,values,str(values.get("trade_time") or ""),None); return
        quote["price"]=float(values.get("price")); self.status["trade_count"]+=1
class Base:
    State=State; merged_event_values=staticmethod(merged)
install(Base)
state=State()
state._apply_trade({"type":"trade","ts":"2026-07-22T10:15:02+09:00","stock_code":"000660","received_code":"000660","kwargs":{"price":"90","trade_time":"101459","source_code":"000660"}})
