...
class State:
    ...
    def _apply_trade(self, event):
        ...
        if cumulative not in (None, ""):
            quote["trade_value_eok"] = float(cumulative)
...
state._apply_trade(
    {
        ...
        "kwargs": {
            ...
            "raw": {
                "price_raw": "110",
                "trade_time_raw": "101501",
                "cumulative_value_raw": "90",
            },
        },
    }
)
...
assert state.status["trade_field_regression_suppressed_count"] == 1
...
print("cumulative_regression_price_preserved")
'''
    )


def test_older_fid20_still_uses_whole_event_drop_and_reason_counter():
    _run(
        r'''
import sys
import threading
    ...
    def _apply_trade(self, event):
        ...
        if incoming is not None and incoming < quote["_trade_time_seconds"]:
            fake_guarded._drop_trade(
                self, quote, event["stock_code"], "older_fid20_than_last_accepted",
                event, values, str(values.get("trade_time") or ""), None,
            )
            return
...
state._apply_trade(
    {
        "type": "trade",
        "stock_code": "000660",
        "kwargs": {"price": "90", "trade_time": "101459", "cumulative_value": "101"},
    }
)
assert state.quotes["000660"]["price"] == 100
assert state.status["dropped_trade_count"] == 1
assert state.status["dropped_trade_reason_counts"] == {
    "older_fid20_than_last_accepted": 1
}
print("older_fid20_still_dropped")
'''
    )


def test_production_entrypoint_installs_trade_field_regression_guard():
    _run(
        r'''
import importlib
...
assert getattr(guarded.base.State, "_stockboard_trade_field_regression_guard_version", None) == "trade_field_regression_guard_v1"
print("trade_field_regression_guard_production_ok")
'''
    )
