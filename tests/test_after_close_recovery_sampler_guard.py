...
from realtime_v2.after_close_theme_recovery import _format_eok  # noqa: E402
...
def test_zero_volume_minute_bars_are_no_trade_not_missing():
    ...
    assert result["minute_recovery_trade_status"] == "no_trade"
    assert result["minute_recovery_display_text"] == "거래없음"
    assert result["minute_trade_value_1m_eok"] == 0
    metadata = result["recovery_values"]["trade_value_1m_eok"]
    assert metadata["source"] == "MINUTE_BAR_NO_TRADE"
    assert metadata["status"] == "NO_TRADE"
    assert metadata["is_estimated"] is False


def test_no_trade_formatter_is_shared_by_theme_recovery():
    assert _format_eok(0, False, False) == "거래없음"
    assert _format_eok(0, True, True) == "거래없음"
...
def test_runtime_installs_sampler_guard_on_both_processes():
    ...
    assert "install_theme_format_guard()" in platform
