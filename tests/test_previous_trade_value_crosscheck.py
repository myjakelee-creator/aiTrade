...
def test_universe_sanitizer_rejects_explicit_value_without_independent_crosscheck():
    payload, rejected = sanitize_payload(
        {
            "items": [
                {
                    "stock_code": "000001",
                    "seed_rank": 1,
                    "seed_trade_value_eok": 5000,
                    "prev_trade_value_eok": 2000,
    ...
    assert rejected == {"000001"}
    assert item.get("prev_trade_value_eok") is None
    assert item["prev_trade_value_source"] == "unavailable"
    assert item["prev_trade_value_status"] == "missing_no_independent_crosscheck"
    assert item["prev_trade_value_rejected_value_eok"] == 2000


def test_universe_sanitizer_rejects_hynix_shape_tiny_crosschecked_value():
    payload, rejected = sanitize_payload(
        {
            "items": [
                {
                    "stock_code": "000660",
                    "seed_rank": 1,
                    "seed_trade_value_eok": 76476.9,
                    "prev_trade_value_eok": 0.02,
    ...
    hynix, samsung = payload["items"]
    assert rejected == {"000660"}
    assert hynix.get("prev_trade_value_eok") is None
    assert hynix["prev_trade_value_status"] == "missing_below_plausibility_floor"
    assert hynix["prev_trade_value_conversion_version"] == VERSION
    assert samsung["prev_trade_value_eok"] == 107886.78
    assert samsung["prev_rank"] == 1
    assert payload["previous_trade_value_fail_closed_rejected_count"] == 1
